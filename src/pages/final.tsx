...
export default function Entry() {
  ...
  const generatePdfWithData = async (
    ...
    // Mapeo de valores a coordenadas para las X (1-6)
    const ratingCoords: Record<number, { x: number }> = {
      6: { x: 231 },
      5: { x: 279 },
      4: { x: 327 },
      3: { x: 375 },
    ...
    // Coordenadas Y para cada campo de evaluación
    ...
    // Coordenadas para los demás campos
    ...
    > = {
      ...
      RecomendariaAlAlumno: { x: 53, y: 40, size: 11 },
      Observaciones: { x: 53, y: 15, size: 11 },
    };

    // Dibujar cada campo en el PDF
    for (const [field, value] of Object.entries(row)) {
      if (evaluationFieldsY[field]) {
        // Campos de evaluación (mostrar X según valor)
        const numericValue = parseInt(value);
        if (!isNaN(numericValue) && ratingCoords[numericValue]) {
          firstPage.drawText("x", {
            x: ratingCoords[numericValue].x,
        ...
        // Otros campos (mostrar texto normal)
